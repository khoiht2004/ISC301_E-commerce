...
console.log(
  "[Mail] GMAIL_APP_PASSWORD loaded:",
...
transporter.verify((error, success) => {
  if (error) {
    console.error(
      "[Mail] SMTP connection verification failed on startup:",
      error,
    );
  } else {
    console.log(
      "[Mail] SMTP connection verification success. Server is ready to take messages",
    );
  }
});
  ...
  const htmlContent = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; border: 1px solid #eee; border-radius: 8px; overflow: hidden;">
      <div style="background-color: #ef4444; padding: 20px; text-align: center;">
        <h1 style="color: #ffffff; margin: 0; font-size: 24px;">Deat Lemi Shop</h1>
      </div>
      <div style="padding: 30px; background-color: #ffffff;">
        <h2 style="color: #333333; margin-top: 0;">Welcome to Deat Lemi Shop!</h2>
      ...
      <div style="background-color: #f9f9f9; padding: 15px; text-align: center; border-top: 1px solid #eeeeee;">
        <p style="color: #999999; font-size: 12px; margin: 0;">&copy; ${new Date().getFullYear()} Deat Lemi Shop. All rights reserved.</p>
      </div>
    </div>
  `;

  const mailOptions = {
    from: `"Deat Lemi Shop" <${process.env.GMAIL_USER}>`,
    to: toEmail,
    subject: "Verify your email - Deat Lemi Shop",
    html: htmlContent,
  };

  try {
    console.log(
      "[Mail] Sending verification email via Gmail SMTP to:",
      toEmail,
    );
    const info = await transporter.sendMail(mailOptions);
    console.log(
      "[Mail] Verification email sent successfully, messageId:",
      info.messageId,
    );
    return true;
  } catch (err) {
