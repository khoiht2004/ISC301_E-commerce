...
const verifyWebhookApiKey = (authorizationHeader) => {
  if (!SEPAY_API_KEY) {
    console.warn(
      "[SePay] ⚠️  SEPAY_API_KEY chưa được cấu hình — bỏ qua xác thực webhook",
    );
    return true;
  }
  ...
  if (!isValid) {
    console.error(
      `[SePay] ❌ API key không hợp lệ. Received: "${received?.slice(0, 8)}..."`,
    );
  }
...
const extractOrderCode = (transactionContent) => {
  ...
  if (matchUnderscore) {
    console.log(
      `[SePay] ✅ Format dấu gạch dưới — orderCode: ${matchUnderscore[1]}`,
    );
  ...
  if (matchNoUnderscore) {
    ...
    console.log(
      `[SePay] ✅ Format không dấu gạch dưới — digits: ${digits} → orderCode: ${orderCode}`,
    );
    return orderCode;
  }
  ...
  console.warn(
    `[SePay] ⚠️  Không tìm thấy orderCode trong: "${transactionContent}"`,
  );
  return null;
};
...
const validateWebhookPayload = (body) => {
  ...
  const content =
    body.transactionContent || body.content || body.transferContent;

  if (!amount || Number(amount) <= 0) {
    return {
      valid: false,
      reason: "Not a deposit transaction (invalid or zero amount)",
    };
  }
  ...
  if (!content || typeof content !== "string") {
    return {
      valid: false,
      reason: "Missing transactionContent or equivalent text field",
    };
  }
