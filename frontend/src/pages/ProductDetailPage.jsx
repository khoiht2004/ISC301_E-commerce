  ...
  const fetchProductDetails = useCallback(async () => {
    try {
      ...
      if (res.data?.success) {
        ...
        const allImages = [prod.thumbnail, ...(prod.images || [])].filter(
          Boolean,
        );
              ...
              <div>
                ...
                <div className="flex items-center gap-2 mb-6">
                  ...
                  <span className="text-sm font-medium text-slate-600">
                    {product.stock > 0
                      ? `Còn hàng (Hiện có ${product.stock} sản phẩm)`
                      : "Hết hàng"}
                  </span>
                </div>
            ...
            {loadingRelated ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6">
                {[...Array(4)].map((_, idx) => (
                  <div
                    key={idx}
                    className="bg-white rounded-2xl border border-slate-100 overflow-hidden shadow-sm animate-pulse h-64"
                  ></div>
                ))}
              </div>
            ) : (
                        ...
                        <div>
                          ...
                          <div className="flex flex-wrap gap-1 mb-2">
                            {p.tags?.slice(0, 2).map((t) => (
                              <span
                                key={t.id}
                                className={`text-[8px] font-semibold px-1.5 py-0.5 rounded-full border ${getTagColorClass(t.slug)}`}
                              >
                                {t.name}
                              </span>
                            ))}
