...
const OrderSuccessPage = () => {
    ...
    const fetchOrder = async () => {
      try {
        ...
        const statusRes = await axios.get(
          `/orders/${orderCode}/payment-status`,
        );
  ...
  const orderStatus = statusMap[order.orderStatus] || {
        ...
        <div className="bg-white rounded-3xl border border-slate-100 shadow-sm p-8 text-center relative overflow-hidden">
          ...
          <div className="bg-slate-50 border border-slate-100 rounded-2xl p-6 mb-8 text-left space-y-4">
            ...
            <div className="flex justify-between items-center text-sm">
              ...
              <span
                className={`font-bold px-3 py-1 rounded-full text-xs ${orderStatus.color}`}
              >
                {orderStatus.label}
              </span>
            </div>
            ...
            <div className="flex justify-between items-center text-sm">
              ...
              <span
                className={`font-bold px-3 py-1 rounded-full text-xs ${payStatus.color}`}
              >
                {payStatus.label}
              </span>
            </div>
          ...
          {isCOD && (
            <div className="bg-amber-50 border border-amber-200 rounded-2xl p-4 mb-8 text-left">
              ...
              <p className="text-xs text-amber-700">
                ...
                <strong>1–3 ngày làm việc</strong>. Bạn sẽ nhận được thông báo
