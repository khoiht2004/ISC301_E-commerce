...
const StaffDashboard = () => {
  ...
  const handleComplete = async (requestId) => {
    if (!window.confirm("Bạn có chắc chắn muốn hoàn thành phiên hỗ trợ này?"))
      return;
    setSubmittingAction(true);
    try {
  ...
  const handleClose = async (requestId) => {
    if (!window.confirm("Đóng hoàn toàn ticket này? Sẽ không thể mở lại."))
      return;
    setSubmittingAction(true);
    try {
    ...
    <div className="flex h-screen bg-slate-50 overflow-hidden font-sans text-slate-800">
      ...
      <div className="flex-1 flex flex-col overflow-hidden bg-slate-100">
        {currentModule === "overview" ? (
          ...
          <StaffProductPage />
        ) : currentModule === "soldProducts" ? (
          <StaffProductPage initialTab="sold" />
          ...
          <>
              ...
              <div className="p-4 bg-slate-100 rounded-2xl border border-slate-200/80 max-w-xl mx-auto text-center flex flex-col gap-1.5 shadow-sm">
                ...
                <p className="text-xs text-slate-500 leading-normal">
                  ...
                  {selectedTicket.acceptedAt &&
                    ` và được nhận lúc ${new Date(selectedTicket.acceptedAt).toLocaleTimeString()}`}
                </p>
                {selectedTicket.closedAt && (
                  <div className="text-[10px] text-blue-600 font-extrabold bg-blue-50 py-1.5 px-3 rounded-lg w-max mx-auto border border-blue-200/40">
                    ...
                    {getSupportDuration(
                      selectedTicket.acceptedAt || selectedTicket.createdAt,
                      selectedTicket.closedAt,
                    )}
                    )
                  </div>
                    ...
                    <div className="flex flex-col gap-1 max-w-[70%]">
                      ...
                      >
                        ...
                        <div className="flex justify-between items-center mt-1.5 pt-1 border-t border-white/10">
                          <span
                            className={`text-[9px] ${msg.senderId === user.id ? "text-primary-200/80" : "text-slate-400"}`}
                          >
                          ...
                          {msg.senderId === user.id && (
                            <Check size={10} className="text-primary-200" />
                          )}
                        </div>
                      </div>
                    </div>
            ...
            {selectedTicket.status === "ACTIVE" ? (
              <form
                ...
                className="p-4 bg-white border-t border-slate-200 flex gap-3 shadow-inner shrink-0"
              >
                <input
                  type="text"
                  placeholder="Nhập câu trả lời hỗ trợ khách hàng..."
                  className="flex-1 px-5 py-3.5 bg-slate-100 rounded-xl focus:outline-none focus:ring-2 focus:ring-primary-600 focus:bg-white border border-slate-200 text-sm font-medium transition-all"
                  value={inputText}
                ...
                <button
                  ...
                  className="px-5 bg-primary-700 text-white font-extrabold rounded-xl flex items-center justify-center hover:bg-primary-800 disabled:opacity-50 transition shadow-md"
                >
              ...
              <div className="p-4 bg-slate-100 border-t border-slate-200 text-center text-xs text-slate-500 font-bold shrink-0">
                Phiên hỗ trợ này đã hoàn thành hoặc lưu trữ. Bạn không thể gửi
                thêm tin nhắn.
              </div>
            )}
          </>
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-slate-400 bg-white p-8">
            ...
            <p className="text-sm text-slate-500 text-center max-w-sm mt-2 leading-relaxed">
              Chào mừng bạn đến với hệ thống CRM Ticket. Chọn cuộc trò chuyện
              bên cột trái để xử lý sự cố của khách hàng.
            </p>
