...
const ChatWidget = () => {
  ...
  const handleRequestSupport = () => {
    ...
    if (!socket || !connected) {
      return alert(
        "Đang kết nối đến máy chủ hỗ trợ. Vui lòng đợi trong giây lát!",
      );
    }
    setLoading(true);
    ...
    <div className="fixed bottom-6 right-6 z-[9999] flex flex-col items-end">
      <AnimatePresence>
        {isOpen && (
          ...
          >
            {/* Header */}
            <div className="bg-primary-800 p-4 flex items-center justify-between text-white">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-1.5">
                    <span
                      className={`w-2 h-2 rounded-full ${connected ? "bg-green-400" : "bg-gray-400"}`}
                    />
              ...
              {!user ? (
                <div className="flex-1 flex flex-col items-center justify-center p-8 text-center">
                  ...
                  <p className="text-gray-600 mb-6">
                    Bạn cần đăng nhập để bắt đầu trò chuyện hỗ trợ.
                  </p>
                  <a
                    href="/login"
                    className="w-full py-3 bg-primary-800 text-white font-bold rounded-xl hover:bg-primary-900 transition shadow-lg"
                  >
                    Đăng nhập ngay
                  </a>
                </div>
                ...
                <div className="flex-1 flex flex-col items-center justify-center p-8 text-center">
                  <MessageSquare size={48} className="text-primary-100 mb-4" />
                  <h5 className="font-bold text-gray-800 mb-2">
                    Xin chào {user.fullName}!
                  </h5>
                  <p className="text-sm text-gray-500 mb-8">
                    Chúng tôi luôn sẵn sàng hỗ trợ bạn. Nhấn nút bên dưới để bắt
                    đầu.
                  </p>
                  <button
                    onClick={handleRequestSupport}
                    disabled={loading}
                    className="w-full py-3 bg-primary-800 text-white font-bold rounded-xl hover:bg-primary-900 transition shadow-lg disabled:opacity-50"
                  >
                    {loading ? (
                ...
                <div className="flex-1 flex flex-col items-center justify-center p-8 text-center">
                  <div className="relative mb-6">
                    <Clock size={64} className="text-primary-100" />
                ...
                <>
                  <div className="p-2 bg-white border-b flex items-center gap-2 px-4">
                    ...
                    <span className="text-xs font-medium text-gray-600">
                      Nhà cung cấp{" "}
                      <strong>{request.assignedStaff?.fullName}</strong> đang
                      trả lời
                    </span>
                  </div>
                  <div
                  ...
                  >
                    ...
                    <button
                      type="submit"
                      className="w-10 h-10 rounded-full bg-primary-800 text-white flex items-center justify-center hover:bg-primary-900 transition"
                    >
                      <Send size={18} />
                    </button>
                  </form>
      ...
      <motion.button
        ...
        className="w-16 h-16 bg-primary-800 text-white rounded-full shadow-2xl flex items-center justify-center hover:bg-primary-900 transition-colors"
      >
